...
libboorudl is a C++17 library for api consuming and web scraping of Gelbooru Beta websites using [cURL](https://curl.se/libcurl/) and [pugixml](https://pugixml.org/)
...
Examples can be build by adding `-D<example option>` after `cmake -B build` and setting the target to the example's or `all`
```bash
